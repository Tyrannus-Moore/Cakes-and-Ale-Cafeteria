...
<div class="page-content" style="padding-top: 50px;">
    ...
    <div class="row maintop">
        <div class="col-xs-4 col-sm-2" style="width: 7.666%;">
            <a href="<?php echo U('settlementExport',array('search'=>$search));?>">
        ...
        <div class="col-xs-5 col-sm-3">
            <form name="admin_list_sea" class="form-search" method="post" action="<?php echo U('settlementList');?>">
                <div class="row maintop">
                    <div class="col-xs-12 col-sm-3 btn-sespan">
                        <div class="input-group">
                            <span class="input-group-addon">订单编号</span>
                            <input type="text" name="search[order_no]"  class="form-control" value="<?php echo ($search["order_no"]); ?>" placeholder="订单编号" style="width:180px;" />
                            <span class="input-group-addon">档口名称</span>
                            <select name="search[stall_id]" class="form-control" style="width:100px">
                                <option value="">请选择</option>
                                <?php if(is_array($stall)): $i = 0; $__LIST__ = $stall;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["stall_id"]); ?>" <?php if($search['stall_id'] == $v['stall_id']): ?>selected<?php endif; ?>><?php echo ($v["stall_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                            <span class="input-group-addon">结算类型</span>
                            <select name="search[state]" class="form-control" style="width:170px">
                                <option value="">请选择</option>
                                <option value="1" <?php if($search['state'] == 1): ?>selected<?php endif; ?>>商户结算（订单金额）</option>
                                <option value="2" <?php if($search['state'] == 2): ?>selected<?php endif; ?>>配送员结算（配送费）</option>
                                <option value="3" <?php if($search['state'] == 3): ?>selected<?php endif; ?>>积分结算（运费）</option>
                            </select>
                        ...
                        <div class="input-group" style="padding-top: 10px;">
                            ...
                            <select name="search[statue]" class="form-control" style="width:80px">
                                <option value="">请选择</option>
                                <option value="1" <?php if($search['statue'] == 1): ?>selected<?php endif; ?>>收入</option>
                                <option value="2" <?php if($search['statue'] == 2): ?>selected<?php endif; ?>>支出</option>
                            </select>
                            ...
                            <span class="input-group-btn btn-group-sm" style="padding-left:10px;">
                                <a href="<?php echo U('settlementList');?>">
        ...
        <div class="col-xs-12">
            <div>
                <form id="leftnav" name="leftnav" method="post" action="" >
                    ...
                    <strong>总收入：<?php echo ((isset($totalIncome) && ($totalIncome !== ""))?($totalIncome):0); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong><strong>总支出：<?php echo ((isset($totalExpend) && ($totalExpend !== ""))?($totalExpend):0); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong><strong>净收入：<?php echo ((isset($totalReceipt) && ($totalReceipt !== ""))?($totalReceipt):0); ?></strong>
                    <table class="table table-striped table-bordered table-hover" id="dynamic-table">
                        ...
                        <?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
                                ...
                                <td class="hidden-sm hidden-xs">
                                    <?php if(($v["statue"]) == "1"): ?>收入<?php endif; ?>
                                    <?php if(($v["statue"]) == "2"): ?>支出<?php endif; ?>
                                </td>
                                <td class="hidden-sm hidden-xs">
                                    <?php if(($v["state"]) == "1"): ?>商户结算（订单金额）<?php endif; ?>
                                    <?php if(($v["state"]) == "2"): ?>配送员结算（配送费）<?php endif; ?>
                                    <?php if(($v["state"]) == "3"): ?>积分结算（运费）<?php endif; ?>
                                </td>
                                <td><?php echo (date("Y-m-d H:i:s",$v["creattime"])); ?></td>
                                <td>
                                    <div class="hidden-sm hidden-xs action-buttons">
                                        <a class="btn btn-minier btn-yellow" href="<?php echo U('settlementDetail',array('id'=>$v['id'],'p'=>I('p',1)));?>" title="查看详情">
                                            查看详情
                                        </a>
                                    </div>
                                </td>
