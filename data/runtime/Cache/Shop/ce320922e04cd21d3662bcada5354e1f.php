    ...
    <div class="page-content" style="padding-top: 50px;">
        ...
        <div class="row maintop">
            <div class="col-xs-5 col-sm-9">
                <form name="admin_list_sea" class="form-search" method="post" action="<?php echo U('foodList');?>">
                    <div class="row maintop">
                        <div class="col-xs-12 col-sm-3 btn-sespan">
                            <div class="input-group">
                                <span class="input-group-addon">ID：</span>
                                <input type="text" name="search[dishes_id]"  class="form-control" value="<?php echo ($search["dishes_id"]); ?>" placeholder="请输入ID" style="width:150px;" />
                                <span class="input-group-addon">菜品名称：</span>
                                <input type="text" name="search[dishes_name]"  class="form-control" value="<?php echo ($search["dishes_name"]); ?>" placeholder="请输入菜品名称" style="width:120px;" />
                                <span class="input-group-addon">分类名称：</span>
                                <select name="search[dishes_cate_id]" class="form-control" style="width:80px">
                                    <option value="">请选择</option>
                                    <?php if(is_array($dishes_cate_list)): foreach($dishes_cate_list as $key=>$v): ?><option value="<?php echo ($v['dishes_cate_id']); ?>" <?php if($search['dishes_cate_id'] == $v['dishes_cate_id']): ?>selected<?php endif; ?>><?php echo ($v['cat_name']); ?></option><?php endforeach; endif; ?>
                                </select>
                                <span class="input-group-addon">档口名称：</span>
                                <select name="search[stall_id]" class="form-control" style="width:80px">
                                    <option value="">请选择</option>
                                    <?php if(is_array($stall_list)): foreach($stall_list as $key=>$v): ?><option value="<?php echo ($v['stall_id']); ?>" <?php if($search['stall_id'] == $v['stall_id']): ?>selected<?php endif; ?>><?php echo ($v['stall_name']); ?></option><?php endforeach; endif; ?>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="row maintop">
                        <div class="col-xs-12 col-sm-3 btn-sespan">
                            <div class="input-group">
                                ...
                                <span class="input-group-btn btn-group-sm" style="padding-left:10px;">
                                    <a href="<?php echo U('foodAdd');?>" style="padding-left: 50px">
                    ...
                    <form id="leftnav" name="leftnav" method="post" action="" >
                        <table class="table table-striped table-bordered table-hover" id="dynamic-table">
                            ...
                            <?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
                                    <td><?php echo ($v["dishes_no"]); ?></td>
                                    <td class="hidden-sm hidden-xs"><?php echo ($v["dishes_name"]); ?></td>
                                    <td class="hidden-sm hidden-xs"><?php echo ($v["stall_name"]); ?></td>
                                    <td class="hidden-sm hidden-xs"><?php echo ($v["cat_name"]); ?></td>
                                    <td class="hidden-sm hidden-xs"><?php echo ($v["price"]); ?></td>
                                    <td class="hidden-sm hidden-xs"><?php echo ($v["num"]); ?></td>
                                    <td class="hidden-sm hidden-xs"><?php echo ($v["score"]); ?></td>
                                    <td class="hidden-sm hidden-xs"><?php echo ($v["on_the_pin"]); ?></td>
                                    <td>
                                        <?php if($v["state"] == 2): ?><a class="red state-btn" href="<?php echo U('isSale');?>" data-id="<?php echo ($v["dishes_id"]); ?>" title="下架">
                                                <div id="zt<?php echo ($v["dishes_id"]); ?>">
                                                    <button type="button" class="btn btn-minier btn-danger">下架</button>
                                                </div>
                                            </a>
                                            <?php else: ?>
                                            <a class="red state-btn" href="<?php echo U('isSale');?>" data-id="<?php echo ($v["dishes_id"]); ?>" title="上架">
                                                <div id="zt<?php echo ($v["dishes_id"]); ?>">
                                    ...
                                    <td>
                                        <div class="hidden-sm hidden-xs action-buttons">
                                            <a class="btn btn-minier btn-success"  href="<?php echo U('foodEdit',array('dishes_id'=>$v['dishes_id'],'p'=>I('p',1)));?>" title="编辑">
                                                编辑
                                            </a>
                                            <a class="red confirm-rst-url-btn btn btn-minier btn-danger" data-info="你确定要删除吗？" href="<?php echo U('foodDel',array('dishes_id'=>$v['dishes_id'],'p'=>I('p',1)));?>" title="删除">
                                                删除
                                            </a>
                                        </div>
                                    </td>
