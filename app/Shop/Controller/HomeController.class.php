...
class HomeController extends ShopAuthController{
    ...
    public function addressList(){
        ...
        $address=M('user_address')->where(array('ma_id'=>session('ma_id')))->select();
        $arr = $nav::rule($address);
        $this->assign('address',$arr);//权限列表
        $this->display();
    }
    ...
    public function addressAdd(){
        ...
        if(IS_POST){
            ...
            clear_cache();
            $this->success('添加成功',U('addressList'),1);
        }else{
            $pid = I('pid');
            if($pid == ''){
                $this->display();
            }else{
                $row = $user_address->where(array('id'=>$pid))->field('id,level')->find();
    ...
    public function address_state(){
        $id=I('x');
        $statusone=M('user_address')->where(array('id'=>$id))->getField('status');//判断当前状态情况
        if($statusone==1){
            $statedata = array('status'=>0);
            $auth_group=M('user_address')->where(array('id'=>$id))->setField($statedata);
            clear_cache();
    ...
    public function address_edit(){
        $value = M('user_address')->where(array('id'=>I('id')))->find();
        $this->assign('value',$value);
        $this->display();
    }

    //修改操作
    public function address_runedit(){
        if(!IS_AJAX){
            $this->error('提交方式不正确',U('addressList'),0);
        }else{
            $user_address=M('user_address');
            $user_address->title = I('title');
            $rst = $user_address->where('id='.I('id'))->save();

            if($rst!==false){
                clear_cache();
                $this->success('修改成功',U('addressList'),1);
            }else{
                $this->error('修改失败',U('addressList'),0);
            }
        }
    }
    ...
    public function address_del(){
        //TODO 自动删除子权限
        $rst=M('user_address')->where(array('id'=>I('id')))->delete();
        if($rst!==false){
            clear_cache();
            $this->success('删除成功',U('addressList'),1);
        }else{
            $this->error('删除失败',U('addressList'),0);
        }
    }
