...
class OrderController extends ShopAuthController
{
    ...
    public function paidan()
    {
        ...
        foreach ($arr_order_id as $k => &$v) {
            $res = M('orders')->where("order_id='$v'")->save($data);
        }
    ...
    //打印的数据
    public function prinerOrder(){
        $order_id = I('order_id');
        if(!$order_id){
            $this->error('请选择需要打印的订单！');
        }

        $where['a.order_id'] = array("in",$order_id);
        ...
        $infos = M('orders')->alias('a')
            ...
            foreach ($infos as $k => &$v) {
                $v['row'] = M('order_goods')->field('id,dishes_price,dishes_nums,dishes_name,meal_code,discount_price')->where(array('order_id'=>$v['order_id']))->select();
            }
