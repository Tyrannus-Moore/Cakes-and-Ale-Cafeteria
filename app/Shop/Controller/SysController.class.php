...
class SysController extends ShopAuthController{
	//站点设置显示
	public function sys(){
		$setting_db = M("setting");
		$sys['copyright'] = $setting_db->where("name = 'copyright'")->getField("data");
		$sys['site_tel'] = $setting_db->where("name = 'site_tel'")->getField("data");
		$sys['number'] = $setting_db->where("name = 'number'")->getField("data");
		$sys['version'] = $setting_db->where("name = 'version'")->getField("data");
	...
	public function runsys(){
		if (!IS_AJAX){
			...
			// var_dump(I());
			$setting_db = M("setting");
			$rst = $setting_db->where("name = 'copyright'")->save(array("data"=>I("copyright")));
			$res = $setting_db->where("name = 'site_tel'")->save(array("data"=>I("site_tel")));
			$result = $setting_db->where("name = 'number'")->save(array("data"=>I("number")));
			$rest = $setting_db->where("name = 'version'")->save(array("data"=>I("version")));
			if($rst!==false || $res !== false || $rest !== false || $result !== false){
		...
		if (!IS_AJAX){
			...
			$data = array(
			    ...
			    'THINK_SDK_WEIXIN' => array(
			        'APP_KEY'    => I('weixin_appid'),
			        'APP_SECRET' => I('weixin_appkey'),
	...
	public function admin_runedit(){
        ...
        $admin = M('merchant')->where(array('ma_id'=>$ma_id))->find();
        if ($edit_pwd !== $confirm_pwd){
            $this->error('两次密码输入不一致！',U('profile'),0);
        }
        if (!$admin||encrypt_password($admin_pwd,$admin['ma_pwd_salt']) !== $admin['ma_pwd']){
            $this->error('密码错误，重新输入！',U('profile'),0);
        }
		$res = M('merchant')->where(array('ma_id'=>$ma_id))->setField('ma_pwd',$edit_pwd);
        if($res !== false){
            $this->success('修改成功！',U('profile'),1);
        }else{
            $this->error('修改失败！',U('profile'),0);
        }
	}
	...
	public function profile(){
        $admin = M('merchant')->where(array('ma_id'=>session('ma_id')))->find();
        $shopAddress = "http://".$_SERVER['SERVER_NAME']."/index.php?m=Home&c=Drink&a=index&ma_id=".session('ma_id');
        $stallAddress = "http://".$_SERVER['SERVER_NAME']."/index.php?m=Stall&c=Index&a=index&ma_id=".session('ma_id');
        $this->assign('shopAddress', $shopAddress);
        $this->assign('stallAddress', $stallAddress);
