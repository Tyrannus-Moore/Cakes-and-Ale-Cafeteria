...
class MarkiController extends ShopAuthController
{
    ...
    public function withdrawal_list()
    {
        ...
        $Page= new \Think\Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数
        $page= $Page->show();
        $list = $member_list_db->where($where)->limit($Page->firstRow.','.$Page->listRows)->order("c_time DESC")->select();
		foreach($list as $key=>$val){
			$list[$key]['c_time'] = date("Y-m-d H:i:s" , $val['c_time']);
			$info = M("member_list")->where(array('member_list_id'=>$val['member_list_id']))->find();
			$list[$key]['member_name'] = $info['member_list_nickname'];
		}
